...
import { Avatar, Typography, Space, Button, Tooltip, Popover, Modal, List, Badge } from 'antd';
...
export default function ChatHeader({ room }: ChatHeaderProps) {
  ...
  const menu = (
    <div>
      <Button type="text" block onClick={() => handleMenuClick('members')}>Thành viên</Button>
        ...
        <List
          ...
          renderItem={member => {
            ...
            return (
              <List.Item>
                <List.Item.Meta
                  avatar={<Badge dot={isOnline} color="green"><Avatar src={member.avatarURL} icon={<UserOutlined />} /></Badge>}
                  title={member.name}
