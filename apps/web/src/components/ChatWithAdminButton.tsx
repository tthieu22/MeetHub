...
const ChatWithAdminButton: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // --- Tối ưu: Xử lý rõ ràng các event cho user ---
    const handleSupportRoomPending = (data: { roomId: string }) => {
      api.info({
        key: NOTIF_KEY,
        message: "Yêu cầu hỗ trợ",
        description:
          "Bạn đã gửi yêu cầu chat với admin. Hiện không có admin nào online! Bạn vẫn có thể gửi tin nhắn, admin sẽ phản hồi khi online.",
        placement: "topRight",
        duration: 10,
        onClick: () => {
          if (data.roomId) {
    ...
    const handleSupportRoomAssigned = (data: { roomId: string }) => {
      if (data.roomId) {
        addPopup(data.roomId);
        useChatStore.getState().setCurrentRoomId(data.roomId);
      }
    };
    const handleSupportAdminJoined = (data: { roomId: string }) => {
      if (data.roomId) {
        addPopup(data.roomId);
        useChatStore.getState().setCurrentRoomId(data.roomId);
      }
