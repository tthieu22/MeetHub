...
import { message, Card, Typography, Button, Row, Col, List, Tag, Space } from 'antd';
...
const fieldStyles = [
  ...
  { bg: 'linear-gradient(135deg, #fffbe6 0%, #fff1b8 100%)', border: '#fadb14', hoverBg: '#fff1b8', hoverText: '#d4a017' },
  { bg: 'linear-gradient(135deg, #f9f0ff 0%, #d3adf7 100%)', border: '#722ed1', hoverBg: '#d3adf7', hoverText: '#391085' },
  { bg: 'linear-gradient(135deg, #fff2e8 0%, #ffd8bf 100%)', border: '#fa8c16', hoverBg: '#ffd8bf', hoverText: '#ad6800' },
];
    ...
    <div style={{ 
      ...
      {loading ? (
        ...
        <Card style={{ 
          ...
          <Text style={{ fontSize: '16px', fontWeight: 500 }}>Không tìm thấy phòng</Text>
        </Card>
      ) : (
        <Card
          title={
            ...
            <Col xs={24}>
              <Space direction="vertical" size="large" style={{ width: '100%' }}>
                ...
                >
                  ...
                  <Tag
                    color={
                      ...
                      room.status === 'maintenance' ? '#faad14' : '#722ed1'
                    }
                    style={{ 
                      fontSize: '14px', 
                      padding: '6px 12px', 
                ...
                {room.description && (
                  ...
                  >
                    <Text strong style={{ color: '#1d39c4', fontSize: '16px' }}>Mô tả: </Text>
                    <Paragraph style={{ fontSize: '15px', color: '#595959', lineHeight: '1.6' }}>
                      {room.description}
                    </Paragraph>
                  </Card>
                )}
                  ...
                  >
                    ...
                    <div>
                      {room.features.map((feature, index) => (
                        <Tag
                          key={index}
                          color={['#1890ff', '#eb2f96', '#13c2c2', '#fadb14'][index % 4]}
                          style={{ 
                            margin: '4px', 
                            fontSize: '14px', 
                            padding: '6px 12px', 
                          ...
                          onMouseLeave={(e) => e.currentTarget.style.transform = 'scale(1)'}
                        >
                          <StarFilled style={{ marginRight: '4px' }} /> {feature}
                        </Tag>
                      ))}
                    </div>
                  </Card>
                ...
                {room.devices && room.devices.length > 0 && (
                  ...
                  >
                    <Text strong style={{ color: '#1d39c4', fontSize: '16px' }}>Thiết bị: </Text>
                    <List
                      dataSource={room.devices}
                      renderItem={(device) => (
                        <List.Item style={{ fontSize: '15px', color: '#595959' }}>
                          <Text>
                            {device.name}: {device.quantity} {device.note && <Text type="secondary">({device.note})</Text>}
                          </Text>
                        </List.Item>
                      )}
                    />
                ...
                {room.operatingHours && (
                  ...
                  >
                    <Text strong style={{ color: '#1d39c4', fontSize: '16px' }}>Giờ hoạt động: </Text>
                    <div>
                      ...
                      <br />
                      <Text style={{ fontSize: '15px', color: '#595959' }}>
                        Đóng cửa: {room.operatingHours.close || 'Không xác định'}
                      </Text>
                      {room.operatingHours.closedDays && room.operatingHours.closedDays.length > 0 && (
                        <>
                          <br />
                          <Text style={{ fontSize: '15px', color: '#595959' }}>
                            Ngày đóng cửa: {room.operatingHours.closedDays.join(', ')}
                          </Text>
                        </>
                      )}
                    </div>
                ...
                {room.bookingPolicy && (
                  ...
                  >
                    <Text strong style={{ color: '#1d39c4', fontSize: '16px' }}>Chính sách đặt phòng: </Text>
                    <div>
                      <Text style={{ fontSize: '15px', color: '#595959' }}>
                        Thời gian đặt tối thiểu: {room.bookingPolicy.minBookingHours || 'Không giới hạn'} giờ
                      </Text>
                      <br />
                      <Text style={{ fontSize: '15px', color: '#595959' }}>
                        Thời gian đặt tối đa: {room.bookingPolicy.maxBookingHours || 'Không giới hạn'} giờ
                      </Text>
                      <br />
                      <Text style={{ fontSize: '15px', color: '#595959' }}>
                ...
                {room.cancellationPolicy && (
                  ...
                  >
                    <Text strong style={{ color: '#1d39c4', fontSize: '16px' }}>Chính sách hủy: </Text>
                    <div>
                      <Text style={{ fontSize: '15px', color: '#595959' }}>
                        Thông báo tối thiểu: {room.cancellationPolicy.minNotice || 0} giờ
                      </Text>
                      <br />
                      <Text style={{ fontSize: '15px', color: '#595959' }}>
                        Phí hủy muộn: {room.cancellationPolicy.lateCancelFee || 0} VND
                      </Text>
                    </div>
                  </Card>
                )}
                ...
                >
                  <Text strong style={{ color: '#1d39c4', fontSize: '16px' }}>Cho phép đồ ăn: </Text>
                  <Text style={{ fontSize: '15px', color: '#595959' }}>
                ...
                >
                  <Text strong style={{ color: '#1d39c4', fontSize: '16px' }}>Số lần đặt: </Text>
                  <Text style={{ fontSize: '15px', color: '#595959' }}>{room.bookingCount}</Text>
                </Card>
                {(room.createdAt || room.updatedAt) && (
                  ...
                  >
                    <Text strong style={{ color: '#1d39c4', fontSize: '16px' }}>Thông tin cập nhật: </Text>
                    <div>
                      {room.createdAt && (
                        <>
                          <Text style={{ fontSize: '15px', color: '#595959' }}>
                      ...
                      {room.updatedAt && (
                        <Text style={{ fontSize: '15px', color: '#595959' }}>
                          Cập nhật lúc: {new Date(room.updatedAt).toLocaleString()}
                        </Text>
                      )}
                    </div>
                  </Card>
